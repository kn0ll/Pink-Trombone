...
class TractUI {
    ...
    private _contexts: {
        [id: string]: CanvasRenderingContext2D | null;
    };
    private _canvas: HTMLCanvasElement
    private _context: CanvasRenderingContext2D | null;
    private _tract: TractState;
    private _processor: null | any;
    private _parameters: {
        [id: string]: number;
    };
    private _touchConstrictionIndices: any[];
    ...
    constructor() {
        ...
        this._container.addEventListener("didGetProcessor", (event: UnknownEvent) => {
        ...
        this._canvases.tract.addEventListener("didNewConstriction", (event: UnknownEvent) => {
            this._touchConstrictionIndices[event.detail.touchIdentifier] = event.detail.constrictionIndex;
        });
        this._canvases.tract.addEventListener("didRemoveConstriction", (event: UnknownEvent) => {
    ...
    private _resizeCanvases() {
        for(let id in this._canvases) {
            //this._canvases[id].style.width = this._container.offsetWidth;
            this._canvases[id].style.height = String(this._container.offsetHeight);
        }
    }
    ...
    private _drawCircle(index: number, diameter: number, arcRadius: number) {
        const angle = this._getAngle(index);
        const radius = this._getRadius(index, diameter);
        const context = this._context!;

        context.beginPath();
        context.arc(this._getX(angle, radius), this._getY(angle, radius), arcRadius, 0, 2*Math.PI);
        context.fill();
    }
    private _drawTongueControl() {
        const context = this._context!;
        context.lineCap = context.lineJoin = "round";
        context.strokeStyle = context.fillStyle = "#FFEEF5"; // palePink
        ...
        context.globalAlpha = 0.7;
        context.beginPath();
        context.arc(this._getX(tongueAngle, tongueRadius), this._getY(tongueAngle, tongueRadius), 18, 0, 2*Math.PI);
        context.stroke();
    ...
    private _drawAmplitudes() {
        ...
        for(let index = 2; index < this._processor.tract.length - 1; index++) {
            context.beginPath();
            context.lineWidth = Math.sqrt(this._processor.tract.amplitude.max[index]) *3;
        ...
        for(let index = 1; index < this._processor.tract.nose.length - 1; index++) {
            context.beginPath();
            context.lineWidth = Math.sqrt(this._processor.tract.nose.amplitude.max[index])*3;
    ...
    private _drawPositions() {
        ...
        [
            ...
            const angle = position[0] as number;
            // @ts-ignore
    ...
    private _drawText(index: number, diameter: number, text: string | number, isStraight = true) {
        const angle = this._getAngle(index);
        const radius = this._getRadius(index, diameter);
        const context = this._context!;

        context.save();
        context.translate(this._getX(angle, radius), this._getY(angle, radius) + 2);
    ...
    private _moveTo(index: number, diameter: number) {
        this.__to(index, diameter, true);
    }
    private _lineTo(index: number, diameter: number) {
        this.__to(index, diameter, false);
    }
    private __to(index: number, diameter: number, moveTo = true) {
    ...
    private _getRadius(index: number, diameter: number) {
        var radius = this._tract.radius - this._tract.scale * diameter;

        return radius;
    }

    private _getIndex(x: number, y: number) {
    ...
    private _getDiameter(x: number, y: number) {
        const diameter = ((this._tract.radius - Math.sqrt((Math.pow(x, 2) + Math.pow(y, 2)))) / this._tract.scale);
        return diameter;
    }

    private _isNearTongue(index: number, diameter: number) {
        var isTongue = true;
    ...
    private _setTongue(event: MouseEvent | Touch, position: any) {
