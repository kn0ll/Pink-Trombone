...
class Vector3 {
    ...
    public dot2 = (x: number, y: number) => this.x * x + this.y * y;
  
    public dot3 = (x: number, y: number, z: number) =>
      this.dot2(x, y) + this.z * z;
}
...
type Grad = [number, number, number];
...
class SimplexNoise {
    private grad3 = ([
        ...
        [+0, +1, -1],
        [+0, -1, -1],
    ] as Grad[]).map(vector3Arguments => new Vector3(...vector3Arguments));
    ...
    private perm = new Array(Math.pow(2, 8+1));
    private gradP = new Array(Math.pow(2, 8+1));
    ...
    private simplex2(xin: number, yin: number) {
